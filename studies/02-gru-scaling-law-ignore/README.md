...
### Follow-up: generative fidelity (2026-08-23, r3)

The grid above scores **prediction**. A separate follow-up asked whether the 3-way head
also *generates* realistic ignore trials in a free-running rollout
([r3](analysis/reports/r3-generative-ignore-fidelity.md), D100/H256 × 3 seeds,
~3000 sessions and 1.6M generated trials per run):

5. **It generates ignores, but ~1.6× too many** — 0.151 generated vs 0.096 real.
6. **The defect is failure to *exit* the ignore state**, not to enter it: in the 3×3
   transition matrix the L/R block matches to ±0.02, while `ignore→ignore` is **+0.145**
   and `ignore→L`/`ignore→R` are −0.076/−0.069. Mean ignore run-length 3.55 vs 2.33 trials.
7. **A single generated session is not interpretable.** Whole-session ignore rate spans
   ~0.01–0.9 across RNG seeds *within one session*; seed variance is **4.6×** the
   between-session (embedding) variance (ICC = 0.18). Pool over many rollouts.
8. **The within-session rise is captured** — ignore rate climbs through a session in both
   model and real data, so progressive disengagement is learned; the model just runs hot.

> Verified free-running, not teacher-forced: the rollout reads only
> `curriculum_name`/`n_trials`/`seed`/`subject_id`/session-index — never the animal's
> choices — and model-vs-real ignore overlap is at chance against a circular-shift null
> (mean z = +1.4). Requires the wrapper generative support merged in PR #62.
...
Run `make -C studies/02-gru-scaling-law-ignore` (needs `WANDB_API_KEY`) to regenerate everything:
...
- ✅ **Generative fidelity (r3):** free rollouts vs real behaviour —
  [`analysis/generative/`](analysis/generative/) holds the figures + per-seed CSV. **Not**
  regenerated by `make`: it needs the wrapper generative support (PR #62, on `main`) and a saved
  3-way run dir, then
  `python code/post_training_analysis/ignore_generative_report.py --run-dir <dir> --output-dir <out>`
  (wrapper repo). Session plots: `analysis/generative/plot_example_sessions.py`.
