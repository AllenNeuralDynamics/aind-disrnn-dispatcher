...
| [r3](r3-generative-ignore-fidelity.md) | generative-ignore-fidelity | live | free rollouts DO generate ignores but over-produce ~1.6× (0.151 vs 0.096) and fail to *exit* the ignore state (`ignore→ignore` +0.145); a single rollout is not interpretable (seed variance 4.6× the session effect) |

## Conventions

**r3 is different in kind:** r1/r2 score *prediction* on the trained grid, while
r3 tests *generative fidelity* — free rollouts of saved D100/H256 checkpoints,
compared against real behaviour. It has no `<!-- BEGIN result-N -->` blocks and
is not regenerated by `make`; its numbers come from the wrapper's
`ignore_generative_report.py` (see r3's own frontmatter for the wrapper ref and
HPC job ids).

r1 and r2 are produced by a single script (`analysis/scaling.py`, which pulls
