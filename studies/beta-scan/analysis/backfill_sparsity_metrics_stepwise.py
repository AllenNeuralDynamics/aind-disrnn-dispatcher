...
def fetch_all_checkpoints(s, run_name):
    """Return [(step, ms_params), ...] for EVERY checkpoint in the run's artifact (step-resolved)."""
    an = f"disrnn-output-{run_name}:latest"
    aid = s.post(GQL, json={"query": """query($p:String!,$e:String!,$an:String!){project(name:$p,entityName:$e){artifact(name:$an){id state}}}""",
                            "variables": {"p": PROJECT, "e": ENTITY, "an": an}}, timeout=90).json()["data"]["project"]["artifact"]
    if not aid or aid.get("state") != "COMMITTED":
...
def main():
    ...
    ap.add_argument("--out", default="backfill_sparsity_metrics_stepwise.csv")
    args = ap.parse_args()
    s = _sess()
    runs = list_group_runs(s, args.group)
    finished = {k: v for k, v in runs.items() if v["state"] == "finished"}
    ...
    for rn, meta in sorted(finished.items(), key=lambda kv: (kv[1]["mult"] or 0, kv[1]["beta"] or 0, kv[1]["lr"] or 0, kv[1]["seed"] or 0)):
        ckpts = fetch_all_checkpoints(s, rn)
        if not ckpts:
            print(f"  SKIP {rn[-8:]}: no checkpoints"); continue
        for step, mp in ckpts:
            rec = {"run": rn, "step": step, **{k: meta[k] for k in ("mult", "beta", "lr", "seed", "heldout_eval_ll")}}
            for fam, key in FAMKEYS.items():
                ...
                for mk, mv in fm.items():
                    rec[f"{fam}.{mk}"] = mv
            rows.append(rec)
        mult_=meta["mult"]; beta_=meta["beta"]; lr_=meta["lr"]; seed_=meta["seed"]
        print(f"  {rn[-8:]} mult={mult_} b={beta_} lr={lr_} seed={seed_} -> {len(ckpts)} checkpoints")
    if rows:
        ...
        with open(args.out, "w", newline="") as f:
            ...
            for r in rows:
                w.writerow(r)
        print(f"wrote {len(rows)} rows ({len(finished)} runs) to {args.out}")
    else:
        print("no rows")
