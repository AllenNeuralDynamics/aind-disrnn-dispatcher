# Variant rl-baseline-bari

**Agent.** Bari 2019 — `ForagerQLearning`, 1 learning rate, 1 forget rate, one-step choice kernel, softmax (`L1F1_CK1`).

**Fitted parameters.** `learn_rate`, `forget_rate_unchosen`, `choice_kernel_relative_weight`, `choice_kernel_step_size`, `biasL`, `softmax_inverse_temperature`
...
Same agent as the earlier `rl-baseline-simple`, so its held-out numbers should **reproduce** r8's Bari result — a built-in cross-check on this launch.
