# h128-dscan-rt-only — reaction-time-only arm

Experiment `01M0QQC7WX5NTKDKJV50SM99HQ` (15 tasks: subject_ratio {0.016, 0.049,
0.163, 0.489, 1.0} x seed {0,1,2}), W&B group `h128-dscan-rt-only`, low priority
+ preemptible on both S3-capable H200 clusters.

Priority 4 of 4. Knobs byte-identical to `h128-dscan`; only
`data.timing_features.reaction_time=true` / `lick_counts=false` differ. Both
...
This arm adds **1** input channel; licks-only adds 2; the combined arm adds 3.
