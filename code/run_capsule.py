...
2. Mice snapshot data (pre-saved .pkl files, subject slice 0–6, mature sessions only)
    ```bash
    python code/run_capsule.py data=mice_snapshot model=disrnn
    ```
    Override subject slice or curricula:
    ```bash
    python code/run_capsule.py data=mice_snapshot data.subject_start=0 data.subject_end=10
    ```
