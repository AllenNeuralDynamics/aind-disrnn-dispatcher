...
1. Default single run with the default parameters (data=mice, model=disrnn)
    ...
    Explicit form:
    ```bash
    python code/launch_CO_wrapper.py data=mice model=disrnn
    ```
...
2. Synthetic data generated by an RL agent performing an uncoupled_block task
    ...
    ```bash
    python code/launch_CO_wrapper.py data=synthetic data.synthetic.agent=rl data.synthetic.task=uncoupled_block model=baseline_rl
    ```
...
3. Hydra multirun sweep over model penalties beta and training learning rate
    ...
    ```bash
    python code/launch_CO_wrapper.py -m model.penalties.beta=0.0001,0.001,0.01 model.training.lr=0.0001,0.001
    ```
...
* To call programmatically from Python:
  ```python
  from launch_CO_wrapper import generate_jobs_with_args
...
def generate_jobs_with_args(override_list: list[str]):
    ...
    sys.argv = ["launch_CO_wrapper.py"] + override_list
    generate_jobs()
